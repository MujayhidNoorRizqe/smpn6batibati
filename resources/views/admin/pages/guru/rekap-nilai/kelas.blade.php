...
@section('title', 'Daftar Murid Rekap Nilai')

@section('content')
<div class="page-content">
    <div class="page-header-card">
        <div>
            <h1>Daftar Murid Kelas {{ $kelas->nama_kelas }}</h1>
            <p>Pilih murid untuk melihat detail nilai ujian semester.</p>
        </div>

        <a
            href="{{ route('guru.rekap-nilai.index', [
    ...
    <div class="report-card">
        ...
        <div class="summary-grid">
            ...
            <div class="summary-card">
                ...
                <strong>{{ $mataPelajarans->count() }}</strong>
            </div>
    ...
    <div class="card">
        <div class="card-header">
            <div>
                <h2>List Murid</h2>
                <p>Klik tombol detail untuk melihat nilai per murid.</p>
            </div>
                    ...
                    <tbody>
                        @foreach ($murids as $murid)
                            <tr>
                                ...
                                <td>{{ $murid->jumlah_nilai }} / {{ $murid->jumlah_mapel }}</td>
                                <td>
                                    <strong>{{ $murid->rata_rata ?? '-' }}</strong>
                                </td>
                                <td>
                                    @if ($murid->rata_rata !== null)
                                        <span class="badge-soft">
                                            {{ $murid->predikat }} - {{ $murid->keterangan_predikat }}
                                        </span>
                                    @else
                                ...
                                <td>
                                    @if ($murid->status_nilai === 'Lengkap')
                                        <span class="badge-success">Lengkap</span>
                                    @elseif ($murid->status_nilai === 'Sebagian')
                                        <span class="badge-warning">Sebagian</span>
                                    @else
                                        <span class="badge-danger">Belum Diisi</span>
                                    @endif
                                </td>
                                <td>
                                    <a
                                        href="{{ route('guru.rekap-nilai.murid', [
                                            'murid' => $murid->id,
                                            'kelas_id' => $kelas->id,
                                            'tahun_ajaran_id' => $tahunAjaran->id,
                                            'semester_id' => $semester->id,
